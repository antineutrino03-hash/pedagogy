...
For practical purposes, the $\tau_{min}$ chosen to be $5 \times 10^{-5}$ based on visual judgement of the glassy modulus. However, we noticed that the broaded fit to the dynamic moduli data can be improved and hence and a model fitting performed using the lmfit package which showed different values of the mBSW parameters than the ones published in the paper. This occurs as the authors have obtained these parameters by fitting the spectrum to the prony series obtained from moduli instead of fitting directly to the data. Note that the fitting here is performed using the $w=S$ weighting in the residual definition as there was no available estimation of uncertainties.
